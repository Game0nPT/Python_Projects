...
lower_range2 = np.array( np.clip( targetColor - margin, 0, 255 ), dtype=np.uint8)
upper_range2 = np.array( np.clip( targetColor + margin, 0, 255 ), dtype=np.uint8)
...
while (True):
    ...
    C = [ct for ct in contours if ct.size > 150]
